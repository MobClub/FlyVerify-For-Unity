...
public class FlyVerifyPostProcessBuild
{
    static ArrayList platformJsList;
    static string flyVerifyKey;
    //[PostProcessBuild]
#if UNITY_IOS
    ...
    public static void onPostProcessBuild(BuildTarget target, string targetPath)
    {
        ...
        //拉取配置文件中的数据
        FlyVerifyXCodeEditorModel xcodeModel = new FlyVerifyXCodeEditorModel();
        xcodeModel.flyVerifyKey = flyVerifyKey;
        ...
        foreach (FlyVerifyPathModel pathModel in xcodeModel.folders)
        {
    ...
    private static void initAppKeyInfo()
    {
        ...
        if (projectFileInfo.Exists)
        {
            ...
            string appKey = (string)datastore["FlyVerifyKey"];
            flyVerifyKey = appKey;
        }
        else
        {
    ...
    //添加系统Framework
    private static void AddSysFrameworks(FlyVerifyXCodeEditorModel xcodeModel, PBXProject xcodeProj, string xcodeTargetGuid, string xcodeTargetPath)
    {
    ...
    // flyverify 不涉及该方法
    private static void AddXcodeModelFrameworks(FlyVerifyXCodeEditorModel xcodeModel, PBXProject xcodeProj, string xcodeTargetGuid, string xcodeTargetPath)
    {
        foreach (FlyVerifyPathModel pathModel in xcodeModel.frameworks)
        {
...
#else
                xcodeProj.AddFileToBuild(xcodeTargetGuid, xcodeProj.AddFile(frameworkPath.Substring(1), "FlyVerify" + frameworkPath, PBXSourceTree.Absolute));
#endif
                //将 build setting 设置
    ...
    //添加 BuildSettings
    private static void AddBuildSettings(FlyVerifyXCodeEditorModel xcodeModel, PBXProject xcodeProj, string xcodeTargetGuid)
    {
    ...
    // 添加其他资源
    // flyverify 不涉及该方法
    private static void AddOtherFile(string secondFilePath, string xcodeTargetPath, string xcodeTargetGuid, FlyVerifyPathModel pathModel, PBXProject xcodeProj, Hashtable fileFlags)
    {
        ...
        foreach (string lastFilePath in secondDirectories)
        {
                ...
                !lastFilePath.Contains(".meta"))
            {
                ...
                if (fileFlags.ContainsKey(fileName))
                {
                    ...
                    //Debug.Log(flag);
                    xcodeProj.AddFileToBuildWithFlags(xcodeTargetGuid, xcodeProj.AddFile(otherFilePath.Substring(1), "FlyVerify" + otherFilePath, PBXSourceTree.Absolute), flag);
                }
                else
                {
#if UNITY_2017_1_OR_NEWER
                    xcodeProj.AddFileToBuildSection(xcodeTargetGuid, xcodeProj.AddFrameworksBuildPhase(xcodeTargetGuid), xcodeProj.AddFile(otherFilePath.Substring(1), "FlyVerify" + otherFilePath, PBXSourceTree.Absolute));

#else
                    xcodeProj.AddFileToBuild(xcodeTargetGuid, xcodeProj.AddFile(otherFilePath.Substring(1), "FlyVerify" + otherFilePath, PBXSourceTree.Absolute));
#endif
                }
            }
        }
    ...
    //添加 .h文件
    private static void AddHeader(string secondFilePath, string xcodeTargetPath, string xcodeTargetGuid, FlyVerifyPathModel pathModel, PBXProject xcodeProj)
    {
        ...
        foreach (string lastFilePath in secondDirectories)
        {
            ...
            if (!lastFilePath.Contains(".framework"))
            {
                ...
                //将.h 加入 proj中
#if UNITY_2017_1_OR_NEWER
                xcodeProj.AddFileToBuildSection(xcodeTargetGuid, xcodeProj.AddResourcesBuildPhase(xcodeTargetGuid),xcodeProj.AddFile(headerPath.Substring(1), "FlyVerify" + headerPath, PBXSourceTree.Absolute));
#else
                xcodeProj.AddFileToBuild(xcodeTargetGuid,xcodeProj.AddFile(headerPath.Substring(1), "FlyVerify" + headerPath, PBXSourceTree.Absolute));
#endif
                Debug.LogWarning("===========================");
    ...
    //添加 .a文件
    private static void AddStaticLibrary(string secondFilePath, string xcodeTargetPath, string xcodeTargetGuid, FlyVerifyPathModel pathModel, PBXProject xcodeProj)
    {
        ...
        foreach (string lastFilePath in secondDirectories)
        {
            ...
            //将.a 加入 proj中
#if UNITY_2017_1_OR_NEWER
            xcodeProj.AddFileToBuildSection(xcodeTargetGuid, xcodeProj.AddFrameworksBuildPhase(xcodeTargetGuid), xcodeProj.AddFile(staticLibraryPath.Substring(1), "FlyVerify" + staticLibraryPath, PBXSourceTree.Absolute));
#else
            xcodeProj.AddFileToBuild(xcodeTargetGuid, xcodeProj.AddFile(staticLibraryPath.Substring(1), "FlyVerify" + staticLibraryPath, PBXSourceTree.Absolute));
#endif
            //将 build setting 设置
    ...
    //添加 bundle
    private static void AddBundle(string secondFilePath, string xcodeTargetPath, string xcodeTargetGuid, FlyVerifyPathModel pathModel, PBXProject xcodeProj)
    {
        SearchOption searchOption;
        ...
        foreach (string lastFilePath in secondDirectories)
        {
            ...
            //将 framework 加入 proj中
#if UNITY_2017_1_OR_NEWER
            xcodeProj.AddFileToBuildSection(xcodeTargetGuid, xcodeProj.AddResourcesBuildPhase(xcodeTargetGuid), xcodeProj.AddFile(bundlePath.Substring(1), "FlyVerify" + bundlePath, PBXSourceTree.Absolute));
#else
            xcodeProj.AddFileToBuild(xcodeTargetGuid, xcodeProj.AddFile(bundlePath.Substring(1), "FlyVerify" + bundlePath, PBXSourceTree.Absolute));
#endif
        }
    }

    //添加 Framework
    private static void AddFramework(string secondFilePath, string xcodeTargetPath, string xcodeTargetGuid, string xcodeFrameworkTargetGuid, FlyVerifyPathModel pathModel, PBXProject xcodeProj, ref bool hasMobFramework, FlyVerifyXCodeEditorModel editModel)
    {
        ...
        var filteredDirectories = allDirectories.Where(dir => {
            ...
            bool hasXCFramework = extensions.Any(ext => dirName.EndsWith(".xcframework"));
            if (hasXCFramework)
            {
                return true;
            }

            bool hasFramework = extensions.Any(ext => dirName.EndsWith(".framework"));
            if (hasFramework && dir.Contains(".xcframework"))
            {
                return false;
            }
        ...
        foreach (string lastFilePath in secondDirectories)
        {
            ...
            bool isMOBFoundation = (frameworkName.Contains("MOBFoundation") || frameworkName.Contains("FlyVerifyCSDK"));
            if (!isMOBFoundation || (isMOBFoundation && !hasMobFramework))
            {
                ...
                string fileGuid = (string)xcodeProj.AddFile(frameworkPath.Substring(1), "FlyVerify" + frameworkPath, PBXSourceTree.Absolute);
    ...
    //在info.plist中添加 Appkey AppSecret
    private static void AddAPPKey(PlistElementDict plistElements)
    {
        ...
        if (projectFileInfo.Exists)
        {
            ...
            Hashtable datastore = (Hashtable)MiniJSON.jsonDecode(contents);
            string appKey = (string)datastore["FlyVerifyKey"];
            string appSecret = (string)datastore["FlyVerifySecret"];
            plistElements.SetString("FlyVerifyKey", appKey);
            plistElements.SetString("FlyVerifySecret", appSecret);
        }
        else
        {
            Debug.LogWarning("FlyVerify.keypds no find");
    ...
    private static void AddPermissions(FlyVerifyXCodeEditorModel xcodeModel, PlistElementDict plistElements)
    {
    ...
    //在info.plist中添加 白名单
    private static void AddLSApplicationQueriesSchemes(FlyVerifyXCodeEditorModel xcodeModel, PlistElementDict plistElements)
    {
    ...
    private static void AddURLSchemes(FlyVerifyXCodeEditorModel xcodeModel, PlistElementDict plistElements)
    {
    ...
    private static void AddInfoPlistSet(FlyVerifyXCodeEditorModel xcodeModel, PlistElementDict plistElements)
    {
    ...
    // flyverify 暂不涉及
    private static void AddCapability(FlyVerifyXCodeEditorModel xcodeModel, PBXProject xcodeProj, string xcodeTargetGuid, string xcodeTargetPath)
    {
